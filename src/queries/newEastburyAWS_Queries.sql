...
WHERE u.userID=700107173; /*Replace userID value with whichever userID value should be viewed*/
...
WHERE userID=700101657; /*Replace userID value with whichever userID value should be viewed*/
...
JOIN
course c ON d.deptID=c.deptID
WHERE d.deptID='D01' /*Department ID gets inserted between ''*/
) AS catalog;
...
WHERE courses.courseID='BE201' /*Optional WHERE clause for individual classes, remove line for complete PreReq list*/
...
WHERE studentID='700100828'  /*Student's ID number gets input on this line, remove WHERE clause to get entire table (Admin Version)*/
...
WHERE sHolds.studentID='700727715'; /*Student's ID number gets input on this line, remove WHERE clause to get entire table (Admin Version)*/
...
(SELECT facID.facultyID, u.firstName, u.lastName, u.phoneNumber FROM user u
	...
	(SELECT facultyID FROM advisor
		WHERE studentID=700903061) AS facID /*student's ID number gets input on this line*/
	WHERE facID.facultyID=u.userID) AS adv
ON adv.facultyID=l.userID;
...
(SELECT facList.studentID, u.firstName, u.lastName, u.phoneNumber FROM user u
	...
	(SELECT studentID FROM advisor
		WHERE facultyID=900854299) AS facList /*faculty's ID number gets input on this line*/
	WHERE facList.studentID=u.userID) AS students
ON students.studentID=l.userID
ORDER BY students.lastName ASC;
